...
var render = {
  ...
  initList: function initList() {
    ...
    each(this.images, function (image, i) {
      ...
      var url = options.url;
      var isVideo = Boolean(image.getAttribute('data-video'));
      if (isString(url)) {
      ...
      if (src || url) {
        items.push('<li>' + '<img' + (' src="' + (src || url) + '"') + ' role="button"' + ' data-action="view"' + (' data-index="' + i + '"') + (' data-original-url="' + (url || src) + '"') +(' data-video="' + isVideo + '"')+ (' alt="' + alt + '"') + '>' + '</li>');
      }
    });
    ...
    each(list.getElementsByTagName('img'), function (image) {
      ...
      addListener(image, EVENT_LOADEDMETADATA, proxy(_this.loadImage, _this), {
        once: true
      });
    });
